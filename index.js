const baseDeDdatos = [
  ...
  baseDeDdatos.forEach((element) => {
    let botonSubestaciones = `
      <div>
        <p>
          <a
            class="btn btn-warning seWidth"
    ...
    // ---------- INICIO INTERVALO NIVEL 1 SUBESTACIONES ---------- //
  
    //  ---------- INICIO NIVEL 2 SECCIONES ---------- //
  
    if (element.secciones != 0) {
      for (var i = 1; i <= element.secciones; i++) {
        botonSubestaciones += `
        <div class="card card-body bg-none border-0 marginSecc p-0">
          <div>
            <p>
              <a
                class="btn btn-primary"
                data-bs-toggle="collapse"
                href="#collapse${element.id}Secc${i}"
                role="button"
                aria-expanded="false"
                aria-controls="collapseExample"
              >
            ...
            <div class="collapse" id="collapse${element.id}Secc${i}">`;
  
        // ---------- INICIO INTERVALO NIVEL 2 SECCIONES ---------- //
          ...
          botonSubestaciones += `
              <div class="card card-body bg-none border-0 ms-5 p-0">
                  <div>
                    <p>
                      <a
                        class="btn btn-danger equiposWidth"
                        data-bs-toggle="collapse"
                        href="#collapse${element.id}Secc${i}equipo${element2}"
                        role="button"
                        aria-expanded="false"
                        aria-controls="collapseExample"
                      >
                  ...
                  <div class="collapse" id="collapse${element.id}Secc${i}equipo${element2}">
            `;
  
          // ---------- INICIO INTERVALO NIVEL 3 EQUIPOS  ---------- //
  
          //  ---------- INICIO NIVEL 4 PDF ---------- //
  
          botonSubestaciones += `
                          <p>
                            <img class="img-size border border-1 border-danger bg-dark marginEquipos p-2 mb-0 rounded" id:"${element.id}-Secc${i}-equipo_${element2}-pdf" src="https://www.adrianaduelo.com/wp-content/uploads/2015/02/pdf-icon.png" alt="PDF" srcset=""/>
                          </p>
                          `;
  
          //  ---------- FIN NIVEL 4 PDF ---------- //
  
          // ---------- FIN INTERVALO NIVEL 3 EQUIPOS  ---------- //
        ...
        // ---------- FIN INTERVALO NIVEL 2 SECCIONES---------- //
    ...
    //  ---------- FIN NIVEL 2 SECCIONES ---------- //
  
    // ---------- FIN INTERVALO NIVEL 1 SUBESTACIONES ---------- //
    ...
    App.innerHTML += botonSubestaciones;
  });
